...
try {
	...
	$VersionFiles = @(
		@{
			Path        = "Unity-SoA-Generator/Assets/root/package.json"
			Pattern     = '"version":\s*"[\d\.]+"'
			Replace     = '"version": "{VERSION}"'
			Description = "Unity package version"
		},
		@{
			Path        = "Installer/Assets/Unity SoA Generator Installer/Installer.cs"
			Pattern     = 'public const string Version = "[\d\.]+";'
			Replace     = 'public const string Version = "{VERSION}";'
			Description = "Installer C# version constant"
		},
		@{
			Path        = "Unity-SoA-Generator/Assets/root/README.md"
			Pattern     = "https://github\.com/Saesentsessis/Unity-SoA-Generator/releases/download/[\d\.]+/Unity-SoA-Generator-Installer\.unitypackage"
			Replace     = "https://github.com/Saesentsessis/Unity-SoA-Generator/releases/download/{VERSION}/Unity-SoA-Generator-Installer.unitypackage"
	...
	function Get-CurrentVersion {
		...
		$packageJsonPath = "Unity-SoA-Generator/Assets/root/package.json"
	...
	function Update-VersionFiles {
		...
		foreach ($change in $changes) {
			...
			Set-Content -Path $fullPath -Value $change.Content -NoNewline -Encoding UTF8
		}
    ...
    # This guarantees the directory is restored, even on Ctrl+C
    Pop-Location
}